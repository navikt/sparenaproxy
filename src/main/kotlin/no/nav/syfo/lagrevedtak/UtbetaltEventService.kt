...
class UtbetaltEventService(
    ...
    suspend fun handleUtbetaltEvent(utbetaltEventKafkaMessage: UtbetaltEventKafkaMessage) {
        if (lagreUtbetaltEventOgPlanlagtMeldingService.erBehandletTidligere(utbetaltEventKafkaMessage.utbetalingId)) {
            log.info("Utbetaling med id ${utbetaltEventKafkaMessage.utbetalingId} er behandlet tidligere, ignorerer meldingen")
            return
        }
        MOTTATT_VEDTAK.inc()
