...
class AktiverMeldingServiceTest :
    FunSpec({
        ...
        context("Test av behandleAktiverMelding") {
            ...
            test("Ignorerer melding som er avbrutt") {
                val id = UUID.randomUUID()
                testDb.connection.lagrePlanlagtMelding(
                    opprettPlanlagtMelding(id = id, avbrutt = OffsetDateTime.now(ZoneOffset.UTC))
                )
            ...
            test("Ignorerer melding som er sendt") {
                val id = UUID.randomUUID()
                testDb.connection.lagrePlanlagtMelding(
                    opprettPlanlagtMelding(id = id, sendt = OffsetDateTime.now(ZoneOffset.UTC))
                )
            ...
            test("Avbryter melding hvis bruker er død") {
                ...
                coEvery {
                    syfosyketilfelleClient.harSykeforlopMedNyereStartdato("fnr", any(), any())
                } returns false
                ...
                coVerify(exactly = 0) { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
                val planlagtMelding =
                    testDb.connection.hentPlanlagtMelding("fnr", LocalDate.of(2020, 1, 14)).first()
        ...
        context("Aktivering 8-ukersmelding") {
            ...
            test(
                "Avbryter 8-ukersmelding hvis bruker er sykmeldt, men det er et nytt sykefravær og stansmelding har blitt vurdert (avbrutt)"
            ) {
                val id = UUID.randomUUID()
                ...
                coVerify(exactly = 0) { smregisterClient.erSykmeldt(any(), any()) }
                coVerify(exactly = 0) {
                    syfosyketilfelleClient.harSykeforlopMedNyereStartdato(any(), any(), any())
                }
                coVerify(exactly = 0) { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
            ...
            test(
                "Avbryter 8-ukersmelding hvis bruker er sykmeldt, men det er et nytt sykefravær også når stansmelding ikke er vurdert"
            ) {
                val id = UUID.randomUUID()
                coEvery { smregisterClient.er100ProsentSykmeldt("fnr", any()) } returns true
                coEvery {
        ...
        context("Aktivering 39-ukersmelding") {
            test("Sender 39-ukersmelding og oppdaterer i db hvis bruker fortsatt er sykmeldt") {
                ...
                testDb.connection.lagrePlanlagtMelding(
                    opprettPlanlagtMelding(id = id, type = BREV_39_UKER_TYPE)
                )
            ...
            test("Avbryter 39-ukersmelding hvis bruker har et nyere sykefravær") {
                ...
                coVerify(exactly = 0) { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
                val planlagtMelding =
                    testDb.connection.hentPlanlagtMelding("fnr2", LocalDate.of(2020, 1, 10)).first()
                planlagtMelding.avbrutt shouldNotBeEqualTo null
                planlagtMelding.sendt shouldBeEqualTo null
            }
            test("Sender 39-ukersmelding hvis bruker er sykmeldt og har et eldre sykefravær") {
                ...
                coEvery { smregisterClient.erSykmeldt("fnr2", any()) } returns true
                coEvery {
                    syfosyketilfelleClient.harSykeforlopMedNyereStartdato("fnr2", any(), any())
                } returns false
                testDb.connection.lagrePlanlagtMelding(
                    opprettPlanlagtMelding(
                        id = id,
                ...
                coVerify { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
                val planlagtMelding =
                    testDb.connection.hentPlanlagtMelding("fnr2", LocalDate.of(2020, 5, 10)).first()
                planlagtMelding.sendt shouldNotBeEqualTo null
                planlagtMelding.avbrutt shouldBeEqualTo null
                planlagtMelding.jmsCorrelationId shouldBeEqualTo "correlationId"
            }
            test(
                "Avbryter 39-ukersmelding hvis bruker er sykmeldt, men det er et nytt sykefravær og stansmelding har blitt vurdert (sendt)"
            ) {
                val id = UUID.randomUUID()
                coEvery { smregisterClient.erSykmeldt("fnr2", any()) } returns true
                coEvery {
        ...
        context("Aktivering stansmelding") {
            ...
            test("Avbryter stansmelding hvis bruker har et nyere sykefravær") {
                ...
                coVerify(exactly = 0) { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
                val planlagtMelding =
                    testDb.connection.hentPlanlagtMelding(fnr, LocalDate.of(2020, 1, 10)).find {
                        it.type == STANS_TYPE
                    }!!
                planlagtMelding.avbrutt shouldNotBeEqualTo null
                planlagtMelding.sendt shouldBeEqualTo null
            ...
            test("Sender stansmelding hvis bruker ikke lenger er sykmeldt") {
                ...
                coVerify { arenaMeldingService.sendPlanlagtMeldingTilArena(any()) }
                val planlagtMelding =
                    testDb.connection.hentPlanlagtMelding(fnr, LocalDate.of(2020, 1, 14)).find {
                        it.type == STANS_TYPE
                    }!!
                planlagtMelding.sendt shouldNotBeEqualTo null
                planlagtMelding.avbrutt shouldBeEqualTo null
            ...
            test(
                "Avbryter stansmelding hvis bruker ikke lenger er sykmeldt og det ikke er sendt 4-ukersmelding"
            ) {
                val id = UUID.randomUUID()
                coEvery { smregisterClient.erSykmeldtTilOgMed(fnr, id) } returns null
                testDb.connection.lagrePlanlagtMelding(
            ...
            test("Utsetter stansmelding hvis bruker fortsatt er sykmeldt") {
                ...
                planlagtMelding.sendt shouldBeEqualTo null
                planlagtMelding.avbrutt shouldBeEqualTo null
                planlagtMelding.sendes shouldBeEqualTo
                    tom.plusDays(17)
                        .atStartOfDay()
