    ...
    FunSpec({
        ...
        context("PersonPdlService - erPersonDod") {
            ...
            test("Returnerer false hvis dødsfall-liste er tom") {
                coEvery { pdlClient.getPerson(any(), any()) } returns
                    GetPersonResponse(ResponseData(HentPerson(emptyList())), errors = null)
                coEvery { accessTokenClient.getAccessTokenV2(any()) } returns "token"
            ...
            test("Kaster feil hvis person ikke finnes i PDL") {
                coEvery { pdlClient.getPerson(any(), any()) } returns
                    GetPersonResponse(ResponseData(null), errors = null)
                coEvery { accessTokenClient.getAccessTokenV2(any()) } returns "token"

                assertFailsWith<IllegalStateException> {
                    runBlocking { pdlService.isAlive("123", UUID.randomUUID()) }
                }
            }
        }
    })
